...
void do_fmt_5() {
  ...
  case 6:
    ...
    //
    do_each("BGT");
    if ((regs.PS.Z | (regs.PS.N ^ regs.PS.V)) == 0)
      regs.PC = regs.PC + (dest * 2);
    break;
  case 7:
    ...
    //
    do_each("BLE");
    if ((regs.PS.Z | (regs.PS.N ^ regs.PS.V)) == 1)
      regs.PC = regs.PC + (dest * 2);
    break;
  case 128:
    //      BPL             BRANCH IF PLUS
